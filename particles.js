...
class Color {
    ...
    static random() {
        return new Color(floorto(255 * Math.random(), 1), floorto(255 * Math.random(), 1), floorto(255 * Math.random(), 1));
    }
...
class ParticleSystem {
    ...
    randomize() {
        this.lifeMin = floorto(50 * Math.random(), 1);
        this.lifeMax = floorto((3 * Math.random() + 1) * this.lifeMin, 1);
        this.shape = 1 + Math.floor(14 * Math.random());
        var r = Math.random()/2;
        this.size = new ParticleSetting(floorto(r, 0.01), floorto((1 + Math.random()) * r + Math.random()/2, 0.01), floorto((Math.random()-0.5)/100, 0.001), 0);
        r = Math.random()/2;
        this.speed = new ParticleSetting(floorto(r, 0.01), floorto((1 + Math.random()) * r, 0.01), floorto((Math.random()-0.5)/100, 0.001), 0);
        r = 360 * Math.random();
        this.direction = new ParticleSetting(floorto(r, 0.01), floorto(r + 360 * Math.random(), 0.01), floorto((Math.random()-0.5)*4, 0.01), 0);
        r = 360 * Math.random();
        this.orientation = new ParticleSetting(floorto(r, 0.01), floorto(r + 360 * Math.random(), 0.01), floorto((Math.random()-0.5)*4, 0.01), 0);
        this.alphas = Math.floor(3 * Math.random()) + 1;
        for(var i = 0; i < 3; i++) {
            psys.alpha[i] = floorto(Math.random(), 0.01);
        }
    ...
    generate() {
        return `// Particle code generated by Crystalline\n` +
